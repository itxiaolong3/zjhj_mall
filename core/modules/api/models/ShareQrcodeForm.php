...
//        $editor->blend($goods_qrcode, $wxapp_qrcode, 'normal', 1.0, 'top-left', 470, 1040);
    ...
    public function step_qrcode()
    {
        ...
        if ($this->user) {
            ...
            $editor->resizeExactWidth($user_pic, 68);
            //附加用户头像图片
            $editor->blend($goods_qrcode, $user_pic, 'normal', 1.0, 'top-left', 91, 950);

            // 用户名处理
            $username = $this->setName($user->nickname);
            $namewitch = imagettfbbox(20, 0, $font_path, $username);
            $editor->text($goods_qrcode, $username, 20,(250-$namewitch[2])/2, 1038, new Color('#5b85cf'), $font_path, 0);
           
            $text = '已走了'.$this->num.'步';
            $textwitch = imagettfbbox(20, 0, $font_path, $text);

            $editor->text($goods_qrcode, $text, 20,(250-$textwitch[2])/2, 1073, new Color('#353535'), $font_path, 0);
            unlink($user_pic_path);
        }
        ...
        //
        $editor->text($goods_qrcode, '走路还能赚钱', 25, 472, 994, new Color('#353535'), $font_path, 0);
        $editor->text($goods_qrcode, '长按识别小程序码', 20, 472, 1047, new Color('#999999'), $font_path, 0);

        //调整小程序码图片
        $editor->resizeFit($wxapp_qrcode, 160, 160);
        // 附加小程序码图片
        $editor->blend($goods_qrcode, $wxapp_qrcode, 'normal', 1.0, 'top-center', 0, 950);
